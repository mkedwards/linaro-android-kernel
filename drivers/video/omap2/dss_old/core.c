...
static int dss_get_clocks(void)
{
	...
	if (cpu_is_omap44xx()) {
		...
		r = dss_get_clock(&core.dss_54m_fck, "tv_clk");
		if (r)
			goto err;

		r = dss_get_clock(&core.dss_96m_fck, "video_clk");
		if (r)
			goto err;
...
static int dss_initialize_debugfs(void)
{
	...
	debugfs_create_file("omapdss_dss", S_IRUGO, dss_debugfs_dir,
